...
public class ReadingStatsService {
    ...
    public ReadingStatsResponse getStats() {
        ...
        for (Object[] row : readingRepository.countGroupByStatus()) {
        ...
        Double avg = readingRepository.averageRating();
        double averageRating = (avg == null) ? 0.0 : Math.round(avg * 100.0) / 100.0;

        long totalReviews = readingRepository.countReviews();

        // อ่านจบต่อปี
        List<YearCount> finishedPerYear = readingRepository.countFinishedPerYear().stream()
